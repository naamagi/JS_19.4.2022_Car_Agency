...
// const agencyByName = getAgencyByName(carMarket, "CarMax");
...
const getAgencyIdByName = (carMarket, name) => {
  const agencyByName = getAgencyByName(carMarket, name);
  return agencyByName && agencyByName.agencyId;
};
// console.log(getAgencyIdByName(carMarket, "Best Deal"));

//* getAllAgenciesName
//? @param {}
//? @return {string[]} - agenciesNameArr - Array of all agencies name

const getAllAgenciesNames = (carMarket) => {
  const allAgencies = carMarket.sellers.map((agency) => {
    return agency.agencyName;
  });
  return allAgencies;
};
// console.log("getAgenciesNames: " + getAllAgenciesNames(carMarket));

//* getAllCarToBuy
//? @param {}
//? @return {object[]} - allCarsToBuy - arrays of all cars objects

const getAllCarToBuy = (carMarket) => {
  const carsToBuyArr = [];
  carMarket.sellers.forEach((seller) => {
    seller.cars.forEach((brandObj) =>
      brandObj.models.forEach((modelObj) => carsToBuyArr.push(modelObj))
    );
  });
  return carsToBuyArr;
};
...
// console.log(getAllCarToBuy(carMarket));

//* getAllCarToBuyByAgencyId
//? @param {string} - id of agency
//? @return {object[]} - carsArray - arrays of all models objects of specific agency

const getAllCarToBuyByAgencyId = (carMarket, agencyId) => {
  const carsToBuyArr = [];
  const agencyById = carMarket.sellers.find((agency) => {
    return agencyId === agency.agencyId;
  });
  if (agencyById) {
    agencyById.cars.forEach((brand) =>
      brand.models.forEach((model) => {
        carsToBuyArr.push(model);
      })
    );
    return carsToBuyArr;
  }
...
// console.log(getAllCarToBuyByAgencyId(carMarket, "Plyq5M5A"));
...
const getAllBrandsToBuyAgencyId = (carMarket, agencyId) => {
  const agencyById = carMarket.sellers.find((agency) => {
    return agencyId === agency.agencyId;
  });
  if (agencyById) {
    return agencyById.cars.map((car) => car.brand);
  }
  return undefined;
};
// console.log(
//   `***********************getAllBrandsToBuyAgencyId:*************************`
// );
// console.log(getAllBrandsToBuyAgencyId(carMarket, "Plyq5M5AZ"));
// console.log(getAllBrandsToBuyAgencyId(carMarket, "Plyq5M5A"));
...
//Utility function:
//getCustomerById
const getCustomerById = (carMarket, id) => {
  const customerById = carMarket.customers.find((customer) => {
...
//Utility function:
//getCustomersCars
const getCustomersCars = (carMarket) => {
  const arrayOfCars = [];
  carMarket.customers.forEach((customer) =>
    customer.cars.forEach((car) => arrayOfCars.push(car))
  );
  return arrayOfCars ? arrayOfCars : undefined;
};
...
const getCustomerByName = (carMarket, name) => {
  const customerByName = carMarket.customers.find((customer) => {
    return name.toLowerCase() === customer.name.toLowerCase();
  });
  return customerByName ? customerByName : undefined;
};
// console.log(
//   `***********************getCustomerByName:*************************`
// );
// console.log(getCustomerByName(carMarket, "Ravi Murillo"));
// console.log(getCustomerByName(carMarket, "ravi murillo"));
// console.log(getCustomerByName(carMarket, "Ravi Murilo"));

//* getCustomerIdByName
//? @param {name}
//? @return {String} - customerId - The customer id

const getCustomerIdByName = (carMarket, name) => {
  const customerByName = getCustomerByName(carMarket, name);
  return customerByName ? customerByName.id : undefined;
};
// console.log(
//   `***********************getCustomerIdByName:*************************`
// );
// console.log(getCustomerIdByName(carMarket, "Ravi Murillo"));
// console.log(getCustomerIdByName(carMarket, "ravi murillo"));
// console.log(getCustomerIdByName(carMarket, "Ravi Murilo"));

//* getAllCustomersNames
//? @param {}
//? @return {string[]} - customersNameArr -  Array of all customers name

const getAllCustomersNames = (carMarket) => {
  const allCustomersNames = carMarket.customers.map((customer) => {
...
//   `***********************getAllCustomersNames:*************************`
// );
// console.log(getAllCustomersNames(carMarket));

//* getAllCustomerCars
//? @param {id} - costumerId - costumer id
//? @return {object[]} - customerCarsArr -  Array of all customer cars object

const getAllCustomerCars = (carMarket, id) => {
  const customerById = getCustomerById(carMarket, id);
  return customerById ? customerById.cars : undefined;
};

// console.log(
//   `***********************getAllCustomerCars:*************************`
// );
// console.log(getAllCustomerCars(carMarket, "cnTobUDy6"));
// console.log(getAllCustomerCars(carMarket, "CnTobUDy6"));
...
const getCustomerCash = (carMarket, id) => {
  const customerById = getCustomerById(carMarket, id);
  return customerById ? customerById.cash : customerById;
};

// console.log(`***********************getCustomerCash:*************************`);
// console.log(getCustomerCash(carMarket, "cnTobUDy6"));
// console.log(getCustomerCash(carMarket, "CnTobUDy6"));
...
const setPropertyBrandToAllCars = (carMarket) => {
  carMarket.sellers.forEach((agency) =>
    agency.cars.forEach((car) =>
      car.models.forEach((carModel) => (carModel.brand = car.brand), car)
    )
  );
  const allCarsToBuy = getAllCarToBuy(carMarket);
  const allCustomersCars = getCustomersCars(carMarket);
  allCustomersCars.forEach(
    (car) =>
      allCarsToBuy.forEach((sellersCar) => {
        if (sellersCar.name === car.name) {
          car.brand = sellersCar.brand;
        }
      }),
    allCarsToBuy
  );
...
// );
// setPropertyBrandToAllCars(carMarket);
// console.log(getAllCarToBuy(carMarket));
// console.log(getCustomersCars(carMarket));
...
// );
// console.log(JSON.stringify(setAmountOfCarsToBuyToAllAgencies(carMarket)));
